...
class SpeechesController extends My_Center_Controller
{
	 ...
	 * Result: {"id":"550fc0ac11be313074a8399e","userID":"550ba28c0cde3a0305c027a1","speechID":"550ba28c0cde3a0305c027a1","created_on":{"sec":1427095724,"usec":694000}}
	 */
			...
			{//Check the return result
				...
				{				
 					...
 					$temp = array('id' => $item['_id'] ->__toString(), 'userID' => $item['userID'],
						'speechID' => $item['speechID'], 'created_on' => $item['created_on']);
 					$result[] = $temp;
				}
	...
	public function getOneInterestAction()
	{
		...
		try 
		{
			...
			$result = array('id' => $interestInfo['_id'] ->__toString(), 'userID' => $interestInfo['userID'],
						'speechID' => $interestInfo['speechID'], 'created_on' => $interestInfo['created_on']);
	...
	public function createInterestAction()
	{
		...
		try 
		{
			...
			$newInterest = array('userID' => $userID , 'speechID' => $speechID, 'created_on' => new MongoDate(time()));
	 ...
	 * Result: {"id":"550fd6b811be313074a8399f","userID":"550ba28c0cde3a0305c027a1","speechID":"550faef111be313074a8399c","created_on":{"sec":1427101368,"usec":927000},"stars":5,"comment":"very good"}
	 */
		...
		try
		{
			...
			{//Check the return result
				...
				foreach ($feedbacksinfos as $item)
				{
					...
					$temp = array('id' => $item['_id'] ->__toString(), 'userID' => $item['userID'],
							'speechID' => $item['speechID'], 'created_on' => $item['created_on'], 'stars' => $item['stars'],
	...
	public function getOneFeedbackAction()
	{
		...
		try
		{
			...
			$result = array('id' => $feedbackInfo['_id'] ->__toString(), 'userID' => $feedbackInfo['userID'],
							'speechID' => $feedbackInfo['speechID'], 'created_on' => $feedbackInfo['created_on'], 'stars' => $feedbackInfo['stars'],
	...
	public function createFeedbackAction()
	{
		...
		try
		{
			...
			$newFeedback = array('userID' => $userID , 'speechID' => $speechID, 
					'star' => $star, 'comment' => $comment, 'created_on' => new MongoDate(time()));
	...
	public function getOneCommentAction()
	{
		...
		try
		{
			...
			$result = array('id' => $commentInfo['_id'] ->__toString(), 'userID' => $commentInfo['userID'],
					'speechID' => $commentInfo['speechID'], 'created_on' => $commentInfo['created_on'],
	...
	public function createCommentAction()
	{
		...
		try
		{
			...
			$newComment = array('userID' => $userID , 'speechID' => $speechID,
					 'comment' => $comment, 'created_on' => new MongoDate(time()));
			...
			if(is_null($speechInfo))
				...
				$result = array('id' => $speechInfo['_id'] ->__toString(), 'speakerID' => $speechInfo['speakerID'],
							'subject' => $speechInfo['subject'],'description'=>$speechInfo['description'],
							 'created_on' => $speechInfo['created_on'],
	...
	/**
	 * Author: Shaon
	 * Create a speech via subject, description, speakerID,when,where,fixed,created_on
	...
	public function createSpeechAction()
	{
		...
		try
		{
			...
			$newSpeech = array('subject' => $subject , 'description' => $description,
					...
					'where' => $where,'fixed' => $fixed,'created_on' => new MongoDate(time()));
	...
	/**
	 * Author: Shaon
	 * update a speech via subject, description, speakerID,when,where,fixed,created_on
